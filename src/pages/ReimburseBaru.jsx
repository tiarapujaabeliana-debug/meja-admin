...
export default function ReimburseBaru() {
  ...
  const [lampiran, setLampiran] = useState({ jalur: "", nama: "" });
  ...
  /* Satu lampiran untuk seluruh pengajuan, bukan satu per baris — pemeriksa
     tidak perlu membuka banyak berkas untuk satu pengajuan yang sama.
  ...
  async function pilihLampiran(file) {
    if (!file) return;
    if (!SUPABASE_SIAP) {
    ...
    try {
      ...
      setLampiran({ jalur: izin.jalur, nama: file.name });
    } catch (e) {
  ...
  async function kirim() {
    const draft = {
      ...
      lampiran: lampiran.jalur || null,
      lampiranNama: lampiran.nama || "",
    };
  ...
  return (
    <>
      ...
      <div className="kartu p-4 mt-5">
        <Isian label={t("kLampiran")} wajib htmlFor="lp">
          {lampiran.jalur
            ? <span className="chip">{lampiran.nama || "terlampir"}</span>
            : mengunggah ? <span className="hint">{t("mengunggah")}</span>
              : <label className="btn btn-kecil cursor-pointer" style={{ display: "inline-flex" }}>
                {t("pilihBerkas")}
                <input id="lp" type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png,.heic,.webp"
